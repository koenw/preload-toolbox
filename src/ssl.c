...
int SSL_write(SSL *ssl, const void *buf, int num) {
  int (*original_fn)(SSL*, const void*, int) = dlsym(RTLD_NEXT, "SSL_write");
  ...
  char *log_file = getenv("PRELOAD_SSL_WRITE_LOGFILE");
  if (log_file == NULL) {
    fprintf(stderr, "[preload-ssl]: PRELOAD_SSL_WRITE_LOGFILE variable not set; not writing log\n");
    return ret;
  }
